...
```bash
dinner-optimizer$ sam build --use-container
```
...
```bash
dinner-optimizer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
dinner-optimizer$ sam local start-api
dinner-optimizer$ curl http://localhost:3000/
```
...
```bash
aws cloudformation delete-stack --stack-name dinner-optimizer
```
